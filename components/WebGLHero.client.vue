...
onMounted(async () => {
  ...
  if (!canvas) {
    status.value = 'canvas element missing'
    return
  }
  ...
  try {
    ...
    status.value = `${gl instanceof WebGL2RenderingContext ? 'WebGL2' : 'WebGL1'} renderer initialized`

    contextLostHandler = (event: Event) => {
      event.preventDefault()
      status.value = 'webgl context lost'
    }
    contextRestoredHandler = () => {
      status.value = 'webgl context restored'
    }
    ...
    const testCubeGeometry = new THREE.BoxGeometry(1.6, 1.6, 1.6)
    const testCubeMaterial = new THREE.MeshBasicMaterial({ color: 0xff00ff, wireframe: true })
    disposables.push(testCubeGeometry, testCubeMaterial)
    const testCube = new THREE.Mesh(testCubeGeometry, testCubeMaterial)
    testCube.position.set(mobile ? 1.4 : 3.3, 0, 0)
    scene.add(testCube)
    ...
    const edgeMaterial = new THREE.MeshBasicMaterial({ color: 0x2f9cff, transparent: true, opacity: mobile ? 0.62 : 0.74 })
    const glowMaterial = new THREE.MeshBasicMaterial({ color: 0x1557ff, transparent: true, opacity: 0.22, blending: THREE.AdditiveBlending, depthWrite: false })
    ...
    resizeHandler = () => {
      ...
      status.value = `${gl instanceof WebGL2RenderingContext ? 'WebGL2' : 'WebGL1'} ${Math.round(rect.width)}×${Math.round(rect.height)}`
    }
    ...
    const render = () => {
      ...
      if (!reducedMotion) {
        ...
        testCube.rotation.x = t * 0.7
        testCube.rotation.y = t * 0.9
      }

      renderer?.render(scene, camera)

      if (frames % 60 === 0) {
        status.value = `${gl instanceof WebGL2RenderingContext ? 'WebGL2' : 'WebGL1'} · frames ${frames}`
      }
    ...
    status.value = `init error: ${error instanceof Error ? error.message : String(error)}`
  }
})
...
onBeforeUnmount(() => {
  ...
  const canvas = document.getElementById('webgl-hero-canvas') as HTMLCanvasElement | null
  if (canvas && contextLostHandler) canvas.removeEventListener('webglcontextlost', contextLostHandler)
  if (canvas && contextRestoredHandler) canvas.removeEventListener('webglcontextrestored', contextRestoredHandler)
...
<template>
  ...
  />
  <div style="position:absolute;right:12px;bottom:64px;z-index:6;padding:6px 9px;border:1px solid rgba(255,255,255,.2);background:rgba(0,0,0,.7);color:#fff;font:11px/1.2 monospace;border-radius:6px;pointer-events:none">
    {{ status }}
  </div>
</template>
