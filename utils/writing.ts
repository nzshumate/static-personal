...
export type Article = { slug: string; title: string; description: string; date: string; readingMinutes: number; body: string }
type ArticleMetadata = { title: string; description: string; date: string; published: boolean }
...
export const articles: Article[] = Object.entries(metadata).flatMap(([path, meta]) => {
  ...
  return [{ slug, title: meta.title, description: meta.description, date: meta.date, body, readingMinutes: Math.max(1, Math.ceil(body.trim().split(/\s+/).length / 220)) }]
